...
import io.github.kartoffelsup.realmd.auth.SecureRemotePasswordProtocol.nBigInteger
...
object SecureRemotePasswordProtocol {
  ...
  fun calculateSrp6(accountDto: AccountBean): Srp6Values {
    return calculateSrp6Internal(accountDto)
  }

  internal fun calculateSrp6ForTest(accountDto: AccountBean, b: BigInteger): Srp6Values {
    return calculateSrp6Internal(accountDto, b)
  }

  private fun calculateSrp6Internal(accountDto: AccountBean, b: BigInteger? = null): Srp6Values {
    ...
    val s = sFromDb?.let { BigInteger(it, 16) } ?: BigInteger(sBitSize, ThreadLocalRandom.current())

    val messageDigest = MessageDigest.getInstance("SHA1").apply {
